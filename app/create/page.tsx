...
export default function CreateCampaignPage() {
  ...
  // Calculate potential fundraising amount
  const calculatePotentialFundraising = () => {
    if (!watchedValues.rows || !watchedValues.columns || !watchedValues.pricing_type) {
      return 0;
    }

    const totalSquares = watchedValues.rows * watchedValues.columns;
    
    switch (watchedValues.pricing_type) {
      case 'fixed':
        return totalSquares * (watchedValues.fixed_price || 0);
      
      case 'sequential':
        const start = watchedValues.sequential_start || 0;
        const increment = watchedValues.sequential_increment || 0;
        let total = 0;
        for (let i = 1; i <= totalSquares; i++) {
          total += start + (i - 1) * increment;
        }
        return total;
  ...
  const potentialFundraising = calculatePotentialFundraising();
            ...
            <div className="card space-y-6">
              ...
              <div className="p-4 bg-gray-50 rounded-lg">
                <div className="space-y-2">
                  ...
                  {potentialFundraising > 0 && (
                    <p className="text-sm text-gray-600">
                      Potential fundraising:{" "}
                      <span className="font-medium text-green-600">
                        ${potentialFundraising.toLocaleString()}
                      </span>
                    </p>
                  )}
                  {watchedValues.pricing_type === 'manual' && (
                    <p className="text-sm text-gray-600">
                      <span className="text-blue-600">Manual pricing:</span> Set individual square prices after grid setup
                    </p>
                  )}
                </div>
              </div>
